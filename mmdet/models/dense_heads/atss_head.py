...
class ATSSHead(AnchorHead):
    ...
    def forward_single(self, x, scale):
        ...
        for cls_conv in self.cls_convs:
            if self.with_cp:
                cls_feat = cp.checkpoint(cls_conv, cls_feat)
            else:
                cls_feat = cls_conv(cls_feat)
        for reg_conv in self.reg_convs:
            if self.with_cp:
                reg_feat = cp.checkpoint(reg_conv, reg_feat)
            else:
    ...
    def loss(self,
        ...
        pos_anchors = (ori_anchors, ori_labels, ori_bbox_targets)
        return dict(
            loss_cls=losses_cls,
    ...
    def forward_train(self,
        ...
        if not get_pos_anchors:
            tmp = losses.pop('pos_anchors')
