...
class StandardRoIHead(BaseRoIHead, BBoxTestMixin, MaskTestMixin):
    ...
    def forward_train(self,
        ...
        if self.with_bbox:
            ...
            if hasattr(self, 'co_training'):
                bbox_targets = bbox_results['bbox_targets']
                num_imgs = len(img_metas)
                # labels, label_weights, bbox_targets, bbox_weights
                #ori_anchors = torch.cat([res.bboxes.unsqueeze(0) for res in sampling_results], dim=0)
                max_proposal = 2000
                for res in sampling_results:
                    max_proposal =  min(max_proposal, res.bboxes.shape[0])
                ori_anchors = bbox2roi([res.bboxes for res in sampling_results])
                ori_proposals, ori_labels, ori_bbox_targets, ori_bbox_feats = [], [], [], []
                for i in range(num_imgs):
                    idx = (ori_anchors[:,0]==i).nonzero().squeeze(1)
                    idx = idx[:max_proposal]
                    ori_proposal = ori_anchors[idx][:, 1:].unsqueeze(0)
                ...
                ori_anchors = torch.cat(ori_proposals, dim=0)
                ...
                pos_anchors = (ori_anchors, ori_labels, ori_bbox_targets, ori_bbox_feats)
                losses.update(pos_anchors=pos_anchors)
