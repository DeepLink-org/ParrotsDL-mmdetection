...
class DetrTransformerEncoder(TransformerLayerSequence):
    ...
    def __init__(self, *args, post_norm_cfg=dict(type='LN'), with_cp=6, **kwargs):
...
class Transformer(BaseModule):
    ...
    def __init__(self, encoder=None, decoder=None, init_cfg=None):
        ...
        self.return_enc_intermediate = getattr(self.encoder, 'return_intermediate', False)
    ...
    def forward(self, x, mask, query_embed, pos_embed):
        ...
        if self.return_enc_intermediate:
            return out_dec, memory, enc_inter
        else:
...
@TRANSFORMER_LAYER_SEQUENCE.register_module()
class DeformableRCNNTransformerDecoder(TransformerLayerSequence):
    """Implements the decoder in DETR transformer.
    ...
    def __init__(self, *args, return_intermediate=False, **kwargs):
        ...
        super(DeformableRCNNTransformerDecoder, self).__init__(*args, **kwargs)
        self.return_intermediate = return_intermediate
    ...
    def forward(self,
        ...
        """
        output = query
        reference_points_xyxy = bbox_cxcywh_to_xyxy(reference_points)
        reference = 0
        for img_id in range(len(img_metas)):
            img_h, img_w, _ = img_metas[img_id]['img_shape']
            factor = reference_points.new_tensor([img_w, img_h, img_w,
                                       img_h])
            reference_points_xyxy[img_id] *= factor
        rois = reference_points_xyxy
        ...
        for lid, layer in enumerate(self.layers):
            ...
            if reg_branches is not None:
                if cascade_refine:
                    tmp = reg_branches[lid](output)
                    new_reference_points = bbox_coder.decode(rois, tmp)
                    reference_points_xyxy = new_reference_points.detach()
                    reference_points = new_reference_points
                    for img_id in range(len(img_metas)):
                        img_h, img_w, _ = img_metas[img_id]['img_shape']
                        factor = reference_points.new_tensor([img_w, img_h, img_w,
                                                img_h])
                        reference_points[img_id] /= factor 
                        reference_points_xyxy[img_id][0::2].clamp_(min=0, max=img_w)
                        reference_points_xyxy[img_id][1::2].clamp_(min=0, max=img_h)
                    reference_points = bbox_xyxy_to_cxcywh(reference_points)
                    reference_points.clamp_(min=0, max=1)
                    reference_points = reference_points.detach()
                    '''
                    tmp = reg_branches[lid](output)
                    tmp += reference
                    reference = tmp
                    new_reference_points = bbox_coder.decode(rois, reference)
                    reference_points_xyxy = new_reference_points.detach()
                    reference_points = new_reference_points
                    for img_id in range(len(img_metas)):
                        img_h, img_w, _ = img_metas[img_id]['img_shape']
            ...
            if self.return_intermediate:
                intermediate.append(output)
                intermediate_reference_points.append(reference_points.detach())
...
class DeformableDetrTransformer(Transformer):
    ...
    def __init__(self,
        ...
        self.use_anchor_feats = use_anchor_feats
        self.use_head_feats = use_head_feats
    ...
    def init_layers(self):
        ...
        if self.use_anchor_query:
            self.aux_pos_trans = nn.Linear(self.embed_dims * 2,
                                       self.embed_dims * 2)
            self.aux_pos_trans_norm = nn.LayerNorm(self.embed_dims * 2)
            #self.query_embed = nn.Embedding(300, self.embed_dims)
            if self.use_anchor_feats:
                self.pos_feats_trans = nn.Linear(self.embed_dims, self.embed_dims)
                self.pos_feats_norm = nn.LayerNorm(self.embed_dims)
                if self.use_head_feats:
                    self.pos_feats_trans = nn.Linear(self.embed_dims*49, self.embed_dims)
            if self.num_co_heads > 1:
                self.head_pos_embed = nn.Embedding(self.num_co_heads, self.embed_dims)
    ...
    def forward(self,
            ...
            if return_encoder_output:
                if self.return_enc_intermediate:
                    return inter_states, init_reference_out,\
                        ...
                        enc_outputs_coord_unact, enc_inter
                else:
                    return inter_states, init_reference_out,\
                        inter_references_out, enc_outputs_class,\
        ...
        if return_encoder_output:
            if self.return_enc_intermediate:
                return inter_states, init_reference_out, \
                    inter_references_out, None, None, enc_inter
            else:
                return inter_states, init_reference_out, \
    ...
    def forward_aux(self,
        ...
        if self.num_co_heads > 1:
            ...
            query_pos, query = torch.split(pos_trans_out, c, dim=2)
            #query = self.query_embed.weight[:topk].unsqueeze(0).repeat(query_pos.shape[0], 1, 1)
            if self.use_anchor_feats:
                #query = self.pos_feats_norm(self.pos_feats_trans(pos_feats))
                query = query + self.pos_feats_norm[head_idx](self.pos_feats_trans[head_idx](pos_feats))
                query_pos = query_pos + self.head_pos_embed.weight[head_idx].reshape(1, 1, self.embed_dims)
                #query_pos = query_pos + self.head_pos_embed.weight[head_idx]
        else:
            pos_trans_out = self.aux_pos_trans_norm(
                self.aux_pos_trans(self.get_proposal_pos_embed(topk_coords_unact)))
            query_pos, query = torch.split(pos_trans_out, c, dim=2)
            #query = self.query_embed.weight[:topk].unsqueeze(0).repeat(query_pos.shape[0], 1, 1)
            if self.use_anchor_feats:
                #query = self.pos_feats_norm(self.pos_feats_trans(pos_feats))
                query = query + self.pos_feats_norm(self.pos_feats_trans(pos_feats))
...
# Group Deformable DETR
@TRANSFORMER.register_module()
class GroupDeformableDetrTransformer(Transformer):
    """Implements the DeformableDETR transformer.
    ...
    def __init__(self,
                 ...
                 two_stage_num_proposals=300,
                 **kwargs):
        super(GroupDeformableDetrTransformer, self).__init__(**kwargs)
        self.as_two_stage = as_two_stage
        self.mixed_selection = mixed_selection
        self.num_feature_levels = num_feature_levels
        self.two_stage_num_proposals = two_stage_num_proposals
    ...
    def init_layers(self):
        """Initialize layers of the GroupDeformableDetrTransformer."""
        self.level_embeds = nn.Parameter(
            torch.Tensor(self.num_feature_levels, self.embed_dims))
    ...
    def forward(self,
        ...
        Args:
            ...
            mlvl_masks (list(Tensor)): The key_padding_mask from
                different level used for encoder and decoder,
                each element has shape  [bs, h, w].
            query_embed (list(Tensor)): The query embedding for decoder,
                with shape [num_query, c].
            mlvl_pos_embeds (list(Tensor)): The positional encoding
                of feats from different level, has the shape
                 [bs, embed_dims, h, w].
        ...
        memory = memory.permute(1, 0, 2)
        bs, _, c = memory.shape
        group_query_embed = query_embed
        num_groups = len(group_query_embed)
        group_inter_states, group_init_reference_out, group_inter_references_out = [], [], []
        group_enc_class, group_enc_coord = [], []
        for i in range(num_groups):
            query_embed = group_query_embed[i]
            inter_states, init_reference_out, inter_references_out, enc_outputs_class, enc_outputs_coord = self.forward_single(
                memory,
                mask_flatten,
                spatial_shapes,
                level_start_index,
            ...
            group_inter_states.append(inter_states.permute(0, 2, 1, 3))
            group_init_reference_out.append(init_reference_out)
            group_inter_references_out.append(inter_references_out)
            group_enc_class.append(enc_outputs_class)
            group_enc_coord.append(enc_outputs_coord.sigmoid() if enc_outputs_coord else enc_outputs_coord)

        #inter_states = torch.cat(group_inter_states, dim=0)
        #init_reference_out = torch.cat(group_init_reference_out, dim=0)
        #inter_references_out = torch.cat(group_inter_references_out, dim=0)
        inter_states = group_inter_states
        init_reference_out = group_init_reference_out
        inter_references_out = group_inter_references_out
        enc_outputs_class = group_enc_class
        enc_outputs_coord_unact = group_enc_coord
...
class DABTransformer(BaseModule):
    ...
    def forward_aux(self, x, mask, refpoint_embed, pos_embed, fc_reg, activate, reg_ffn, head_idx):
        """Forward function for `Transformer`.
        ...
        '''
        pos_trans_out = self.aux_pos_trans_norm[head_idx](
            self.aux_pos_trans[head_idx](self.get_proposal_pos_embed(refpoint_embed)))
        query_aux = pos_trans_out
        query_aux = query_aux + self.pos_feats_norm[head_idx](self.pos_feats_trans[head_idx](aux_feats))
        query_aux = query_aux + self.head_pos_embed.weight[head_idx].reshape(1, 1, self.embed_dims)
        pos_embed = query_aux.view(bs, c, -1).permute(2, 0, 1)
        '''
        pos_embed = pos_embed.view(bs, c, -1).permute(2, 0, 1)
        refpoint_embed = refpoint_embed.permute(1, 0, 2) # [bs, num_query, dim] -> [num_query, bs, dim]
        mask = mask.view(bs, -1)  # [bs, h, w] -> [bs, h*w]
        memory = x
        #target = torch.zeros_like(embedweight)
        # query_embed = gen_sineembed_for_position(refpoint_embed)
...
class CTransformer(BaseModule):
    ...
    def __init__(self, encoder=None, decoder=None, init_cfg=None, num_co_heads=0):
    ...
    def forward_aux(self, x, mask, query_embed, pos_embed, aux_anchors, aux_feats, head_idx):
        """Forward function for `Transformer`.
        ...
        aux_anchors = inverse_sigmoid(aux_anchors)
        pos_trans_out = self.aux_pos_trans_norm[head_idx](
            self.aux_pos_trans[head_idx](self.get_proposal_pos_embed(aux_anchors)))
        query_aux = pos_trans_out
        query_aux = query_aux + self.pos_feats_norm[head_idx](self.pos_feats_trans[head_idx](aux_feats))
        query_aux = query_aux + self.head_pos_embed.weight[head_idx].reshape(1, 1, self.embed_dims)
        query_embed = query_aux.permute(1, 0, 2) # [bs, num_query, dim] -> [num_query, bs, dim]
...
@TRANSFORMER.register_module()
class CoDeformableDetrTransformer(Transformer):
    """Implements the DeformableDETR transformer.
    ...
    def forward_aux(self,
        ...
        """
        bs, _, c = memory.shape
        assert self.num_co_heads == len(multi_pos_anchors)
        multi_init_reference_out_aux = []
        multi_reference_points_aux = []
        multi_query_aux = []
        multi_query_pos_aux = []
        for head_idx, pos_anchors in enumerate(multi_pos_anchors):
            pos_feats = multi_pos_feats[head_idx]
            topk_aux = pos_anchors.shape[1]
            topk_coords_unact_aux = inverse_sigmoid((pos_anchors))
            if self.as_two_stage:
                reference_points_aux = pos_anchors
            else:
                reference_points_aux = pos_anchors[:, :, :2]
            init_reference_out_aux = reference_points_aux
            pos_trans_out = self.aux_pos_trans_norm[head_idx](
                self.aux_pos_trans[head_idx](self.get_proposal_pos_embed(topk_coords_unact_aux)))
            query_pos_aux, query_aux = torch.split(pos_trans_out, c, dim=2)
            if self.use_anchor_feats:
                query_aux = query_aux + self.pos_feats_norm[head_idx](self.pos_feats_trans[head_idx](pos_feats))
                query_pos_aux = query_pos_aux + self.head_pos_embed.weight[head_idx].reshape(1, 1, self.embed_dims)
                #query_pos_aux = query_pos_aux + self.head_pos_embed.weight[head_idx]
            multi_init_reference_out_aux.append(init_reference_out_aux)
            multi_reference_points_aux.append(reference_points_aux)
            multi_query_aux.append(query_aux)
            multi_query_pos_aux.append(query_pos_aux)
        init_reference_out_aux = torch.cat(multi_init_reference_out_aux, dim=1)
        reference_points_aux = torch.cat(multi_reference_points_aux, dim=1)
        query_aux = torch.cat(multi_query_aux, dim=1)
        query_pos_aux = torch.cat(multi_query_pos_aux, dim=1)
        ...
        if self.as_two_stage:
            ...
            reference_points = self.reference_points(query_pos).sigmoid()
            init_reference_out = reference_points
        init_reference_out = torch.cat((init_reference_out, init_reference_out_aux), dim=1)
        reference_points = torch.cat((reference_points, reference_points_aux), dim=1)
        query = torch.cat((query, query_aux), dim=1)
        query_pos = torch.cat((query_pos, query_pos_aux), dim=1)
...
class DinoTransformer(DeformableDetrTransformer):
    ...
    def init_layers(self):
        ...
        if self.use_anchor_query:
            self.aux_pos_trans = nn.Linear(self.embed_dims * 2,
                                       self.embed_dims)
            self.aux_pos_trans_norm = nn.LayerNorm(self.embed_dims)
            #self.query_embed = nn.Embedding(300, self.embed_dims)
            if self.use_anchor_feats:
                self.pos_feats_trans = nn.Linear(self.embed_dims, self.embed_dims)
                self.pos_feats_norm = nn.LayerNorm(self.embed_dims)
