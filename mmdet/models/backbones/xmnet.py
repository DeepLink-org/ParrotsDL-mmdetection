...
class LocalBlock(nn.Module):
    def __init__(self, dim, drop_path, init_values=0, norm_cfg=dict(type='MMSyncBN', requires_grad=True)):
        super().__init__()
        self.dw = nn.Conv2d(dim, dim, kernel_size=5, padding=2, groups=dim)
        nn.init.constant_(self.dw.bias, 0)
...
class SwinBlock(BaseModule):
    ...
    def __init__(self,
        ...
        if use_global:
            self.conv = LocalBlock(embed_dims, 0)
...
class XMNet(BaseModule):
    ...
    def __init__(self,
        ...
        super(XMNet, self).__init__(init_cfg=init_cfg)
        ...
        for i in range(num_layers):
            ...
            stage = SwinBlockSequence(
                ...
                norm_cfg=norm_cfg,
                use_global=use_global,
                global_idx=3, #  if i < 3 else 2
                with_cp=with_cp[i] if isinstance(with_cp, tuple) else with_cp,
                init_cfg=None)
    ...
    def init_weights(self):
        ...
        if self.init_cfg is None:
            ...
            if state_dict.get('absolute_pos_embed') is not None and hasattr(self, 'absolute_pos_embed'):
            ...
            '''
            if 'backbone' in relative_position_bias_table_keys[0]:
                pseudo_relative_position_bias_table_keys = [k[9:] for k in relative_position_bias_table_keys]
                state_dict_keys = state_dict.keys()
                for k in state_dict_keys:
                    state_dict[k[9:]] = state_dict.pop(k)
                relative_position_bias_table_keys = pseudo_relative_position_bias_table_keys
            '''
            for table_key in relative_position_bias_table_keys:
                if table_key not in self.state_dict().keys():
                    print('missing: '+table_key)
                    continue
