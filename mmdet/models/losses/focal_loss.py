    ...
    def forward(self,
        ...
        if self.use_sigmoid:
            if self.activated:
                ...
                num_classes = pred.size(1)
                fg_inds = ((target >= 0)
                    & (target < num_classes)).nonzero().squeeze(1)
                labels = target[fg_inds]
                target = F.one_hot(target, num_classes=num_classes + 1)
...
@LOSSES.register_module()
class LabeledFocalLoss(nn.Module):
    ...
    def __init__(self,
        ...
        Args:
            ...
            activated (bool, optional): Whether the input is activated.
                ...
                Defaults to False.
        """
        super(LabeledFocalLoss, self).__init__()
        assert use_sigmoid is True, 'Only sigmoid focal loss supported now.'
        self.use_sigmoid = use_sigmoid
        self.gamma = gamma
        self.alpha = alpha
    ...
    def forward(self,
        ...
        if self.use_sigmoid:
            if self.activated:
                ...
                loss_cls = 0
                num_imgs = len(inds)
                N, num_classes = pred.size(0), pred.size(1)
                preds, targets = pred.reshape(num_imgs, N//num_imgs, num_classes), target.reshape(num_imgs, N//num_imgs)
                weights = weight
                # weights = weight.reshape(num_imgs, N//num_imgs)
                # fg_inds = ((target >= 0)
                #     & (target < num_classes)).nonzero().squeeze(1)
                # labels = target[fg_inds]
                # inds = get_fed_loss_inds(
                ...
                for i in range(num_imgs):
                    pred, target, weight = preds[i], targets[i], weights[i*N:(i+1)*N]
                    ind = (inds[i]>0).nonzero().squeeze(1)
                    target = F.one_hot(target, num_classes=num_classes + 1)
                    target = target[:, ind]
                    pred = pred[:, ind]
                    calculate_loss_func = py_sigmoid_focal_loss

                    loss_cls_tmp = self.loss_weight * calculate_loss_func(
                        pred,
                        target,
                        weight,
                        gamma=self.gamma,
                        ...
                        reduction=reduction,
                        avg_factor=avg_factor)
                    loss_cls = loss_cls + loss_cls_tmp / num_imgs
